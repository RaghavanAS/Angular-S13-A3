...
export class CustomerComponentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (customer) {
      ...
      // invoke the getCityList() and getCustomerList()
  ...
  // on form submit calling the onSave method to add a customer using Customer Service or
  // update the customer based on isNew Flag
  onSave(Values) {
    if (this.isNew) {
    ...
    // navigate to customers page on successful add or update
    this.changesSaved = true;
    this.router.navigate(['/customers']);
  }
  // invoke the canDeactivate method if user tries to naviagte from page during add or update 
